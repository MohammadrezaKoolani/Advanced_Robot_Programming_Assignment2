...
std::shared_ptr<ObstaclesApplication> ObstaclesApplication::make_app(
        const int& domain_id,
        const std::string& entity_kind)
{
    std::shared_ptr<ObstaclesApplication> entity;
    if (strcmp(entity_kind.c_str(), "publisher") == 0)
    {
        entity = std::make_shared<ObstaclesPublisherApp>(domain_id);
    }
    else if (strcmp(entity_kind.c_str(), "subscriber") == 0)
    {
        entity = std::make_shared<ObstaclesSubscriberApp>(domain_id);
    }
    else
    {
